...
- **Note** (`title, content, user_id, parent_id`) — page de note libre en markdown (contenu affiché tel quel côté client via `marked` + `DOMPurify`, jamais interprété côté serveur). `parent_id` permet une seule profondeur de sous-pages : `NoteController` refuse (422) qu'une note dont `parent_id` n'est pas nul devienne elle-même parente, et refuse la suppression d'une note qui a encore des enfants (pas de cascade contrairement aux messages/chanels — l'utilisateur doit d'abord supprimer/déplacer les sous-pages). `user_id` n'est que l'auteur d'origine à titre indicatif : contrairement à la plupart des ressources du référentiel, les notes sont en écriture ouverte à tout utilisateur connecté (`user` ou `admin`), y compris pour modifier ou supprimer une note écrite par quelqu'un d'autre — pas de notion de propriétaire exclusif.
...
- **CRUD complet** sur categories/stations/allergens/ingredient-categories/ingredients/fiche-techniques/menus/pictures/notes (`Route::apiResource`).
...
- **Notes libres en markdown** (`mise-dashboard` page `/notes` sous `components/notes/`, `mise-public` page `/notes` sous `pages/notes/` — composants quasi identiques entre les deux repos car les droits sont symétriques, contrairement à la discussion) :
  - Une page (note racine) peut avoir des sous-pages, une seule profondeur (pas de sous-sous-page) — imposé côté API, pas juste côté UI. Le composant charge tout l'arbre en un seul `GET /api/notes` (racines + enfants imbriqués), pas de pagination (volume de notes attendu trop faible pour la justifier, même logique que `FicheTechnique::index`).
  - CRUD complet ouvert à tout utilisateur connecté (`user` ou `admin`), des deux côtés : n'importe qui peut créer, modifier ou supprimer n'importe quelle page, y compris celles écrites par quelqu'un d'autre — pensé comme un wiki d'équipe partagé, pas des blocs-notes personnels malgré le `user_id`.
  - Éditeur markdown texte brut + aperçu rendu côte à côte (empilés en dessous de 900px) via `core/utils/markdown.ts` (`marked` + `DOMPurify`, dupliqué dans les deux repos comme le reste). Le rendu markdown → HTML n'est fait que côté client, jamais stocké : `content` reste du texte brut en base.
  - Suppression d'une page avec sous-pages bloquée côté API (422) : l'UI cache simplement le bouton de suppression tant que `children.length > 0` plutôt que de gérer l'erreur comme cas nominal.
  - Les styles de rendu markdown (`.markdown-content`, titres/listes/code/citations/tableaux) sont dans le CSS global de chaque app plutôt que dans le CSS scoped du composant — l'encapsulation de style d'Angular ne s'applique pas au HTML injecté dynamiquement via `[innerHTML]`.
...
**Un seul `.env`, à la racine de `mise/`** (copier `mise/.env.example` en `mise/.env`, gitignoré via `/mise/.env` dans le `.gitignore` racine de `dev/`), partagé par Docker (`env_file: .env` sur le service `api`) **et** par une exécution native de `mise-api` hors Docker — `mise-api/bootstrap/app.php` pointe explicitement `useEnvironmentPath()` vers ce fichier un niveau au-dessus (sans effet dans le conteneur : le fichier n'existe pas dans l'image, hors du contexte de build ; les variables y arrivent déjà via `env_file`). `mise-api/.env`/`.env.example` n'existent plus — supprimés lors de cette consolidation. Ajuster `DB_*`/`APP_KEY`/`APP_URL`/`DEMO` selon besoin. Le `docker-compose.yml` du service `api` ne fixe plus qu'une seule variable en dur, `DB_HOST=db` (la seule qui doit réellement différer entre conteneur et exécution native) — tout le reste (drivers session/cache/queue, etc.) vient du `.env`, identique dans les deux contextes.
