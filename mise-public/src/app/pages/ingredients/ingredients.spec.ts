...
describe('Ingredients', () => {
  ...
  it('sorts ingredients by category name (A→Z), then by ingredient name (A→Z), uncategorized last', () => {
    expect(component.sortedIngredients().map((i) => [i.category?.name ?? null, i.name])).toEqual([
  ...
  it('lists the categories present in the data, sorted A→Z', () => {
  ...
  it('filters by the selected category', () => {
    ...
    expect(component.sortedIngredients().map((i) => i.name)).toEqual(['Beurre']);
  });

  it('combines the search text with the selected category', () => {
    component.search.set('œufs');
    expect(component.sortedIngredients().map((i) => i.name)).toEqual(['Œufs']);

    component.onCategoryChange({ target: { value: '2' } } as unknown as Event);
    expect(component.sortedIngredients()).toEqual([]);
  });

  it('resets to all categories when the empty option is chosen', () => {
    component.onCategoryChange({ target: { value: '2' } } as unknown as Event);
    expect(component.sortedIngredients().length).toBe(1);

    component.onCategoryChange({ target: { value: '' } } as unknown as Event);
    expect(component.sortedIngredients().length).toBe(6);
  });
});
